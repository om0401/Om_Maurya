...
export function AboutPage() {
  ...
  const addTool = () => {
    if (newTool.trim() && !editData.tools.includes(newTool.trim())) {
      setEditData({
        ...editData,
        tools: [...editData.tools, newTool.trim()],
      });
      setNewTool("");
    }
  };
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex items-center justify-between">
          ...
          {isOwner && !isEditing && (
            <Button
              onClick={handleEdit}
              className="flex items-center gap-2 bg-primary hover:bg-primary/90 shadow-lg shadow-primary/25"
            >
                ...
                {isEditing ? (
                  <div className="space-y-2">
                    <Input
                      ...
                      placeholder="Enter video URL (e.g., /IMG/1000005230.mp4 or YouTube embed)"
                      className="bg-muted/20 border-border/50"
                    />
                    <p className="text-muted-foreground text-sm">
                      For local file: /IMG/1000005230.mp4 <br />
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex flex-wrap gap-2">
                    {(isEditing ? editData.tools : aboutData.tools).map(
                      (tool, index) => (
                        <Badge
                          key={index}
                          variant="secondary"
                          className="px-4 py-2 bg-primary/10 border border-primary/20 text-primary hover:bg-primary/20 transition-colors"
                        >
                          {tool}
                          {isEditing && (
                            <button
                  ...
                  {isEditing && (
                    <div className="flex gap-2">
                      <Input
                        ...
                        placeholder="Add a tool (e.g., Power BI, React, Python)"
                        onKeyPress={(e) =>
              ...
              <CardContent className="space-y-4">
                {isEditing ? (
                  <div className="space-y-2">
                    ...
                    />
                    <p className="text-muted-foreground text-sm">
                      Upload your resume to Google Drive or GitHub and paste the
                      shareable link here
                    </p>
                  </div>
